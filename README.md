...
```bash
python -m deblurrinator.entropic_deblur              # 1D barcode (default)
python -m deblurrinator.entropic_deblur qr            # QR code
...
python -m deblurrinator.entropic_deblur qr --blur-width 7 --data "TEST" --sigma 1.5
python -m deblurrinator.entropic_deblur --blur-width 31 --m 5 --noise 0.01
python -m deblurrinator.entropic_deblur --no-show     # save PNG only, skip display
```
...
demo(blur_width=21, sigma=1.0)
demo_qr(data="HELLO WORLD", blur_width=5, sigma=1.0, m=5)
```
